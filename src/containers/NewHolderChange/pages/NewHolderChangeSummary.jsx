...
const NewHolderChangeSummary = ({ ...props }) => {
  ...
  const isTariffIndexed = values?.tariff_type === TARIFF_INDEXED
  ...
  const technicalData = [
    ...
    values?.especial_cases === "reason_holder_change"
      ? {
          reviewValue: t("TARIFF_POWER_MODIF_INFO"),
        }
      : {},
  ]
    ...
    <Grid container spacing={4}>
      ...
      <Grid item xs={12}>
        <TermsDialog
          ...
          <PDFLoader
            folder={isTariffIndexed ? "CCGG_indexada" : "CCGG"}
            documentName={
              isTariffIndexed
...
export default NewHolderChangeSummary
