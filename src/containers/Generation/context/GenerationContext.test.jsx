  ...
  return (
    <>
      {assignments.map((assignment) => (
        <li id={assignment.contract} key={assignment.contract}>
          <p>
            {assignment.contract} - {assignment.priority}
          </p>
        </li>
      ))}
      {investments.map((investment) => (
        <li id={investment.name} key={investment.name}>
          {investment.contract}
        </li>
      ))}
      ...
      <Button
        ...
        onClick={() => changeAssigmentPriority(assignments[0], assignments[1])}
      />
...
describe("Generation Context", () => {
  ...
  test("should provide GenerationContext values to child elements", () => {
    ...
    const assignmentElement = getById(
      dom.container,
      mockAssignmentRows[0].contract,
    )
  ...
  test("should change the priority of one assignment", async () => {
    ...
    expect(
      screen.getByText(
        mockAssignmentRows[0].contract + " - " + mockAssignmentRows[1].priority,
      ),
    ).toBeInTheDocument()
    expect(
      screen.getByText(
        mockAssignmentRows[1].contract + " - " + mockAssignmentRows[0].priority,
      ),
    ).toBeInTheDocument()
  })
  ...
  test("should change the priority of one assignment and then reset", async () => {
    ...
    expect(
      screen.getByText(
        mockAssignmentRows[0].contract + " - " + mockAssignmentRows[0].priority,
      ),
    ).toBeInTheDocument()
    expect(
      screen.getByText(
        mockAssignmentRows[1].contract + " - " + mockAssignmentRows[1].priority,
      ),
    ).toBeInTheDocument()
  })
