...
import {
  getAssignmentContracts,
  getNoAssignmentContracts,
} from "../../../services/api"
...
export const GenerationContextProvider = ({
  ...
  const [contractNoAssignments, setContractNoAssignments] = useState(
    contractNoAssignmentsJSON,
  )
  const [investments] = useState(investmentsJSON)

  const getAssingments = useCallback(async () => {
    try {
      let result = await getAssignmentContracts()
      setAssignments(result?.data)
    } catch (err) {
      console.log("No s'han pogut trobar les assignacions", err)
    }
  }, [])
  ...
  const getPriority = useCallback(
    (index) => {
      const priorities = assignments.map((_value, index) => {
        return index === 0
          ? { active: true, value: t("GENERATION_MAIN_PRIORITY"), index: index }
          : {
              active: true,
              value: ordinals.formatOrdinals(index + 1),
              index: index,
            }
      })
      let found = false
  ...
  const getContractsToAssign = useCallback(async () => {
    try {
      if (!testMode) {
