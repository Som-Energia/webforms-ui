...
function GenerationDashboard({
  ...
  const contractListDialog = () => {
    const dataList = contractNoAssignments.map((element) => ({
      id: element.id,
      primary: element.contract,
      secondary: element.contract_address,
    }))
    if (contractNoAssignments.length === 0) {
          ...
          text={
            <Typography
              dangerouslySetInnerHTML={{
                __html: t("GENERATION_ADD_ASSIGNMENTS_INFO_NO_CONTRACTS_TEXT"),
              }}
            />
          }
      ...
      setContent(
        <CustomDialog withBackground={true}>
          <ContractList
            data={dataList}
            title={t("GENERATION_ADD_CONTRACT_LIST_TITLE")}
            acceptFunction={addContracts}
            cancelFunction={() => setContent(undefined)}
            acceptButtonText={t(
              "GENERATION_ADD_CONTRACT_LIST_VALIDATE_CHANGES",
            )}
            cancelButtonText={t("GENERATION_ADD_CONTRACT_LIST_CANCEL")}
          />
        </CustomDialog>,
      )
    }
        ...
        <Grid sx={{ backgroundColor: "background.third" }}>
          {investments.length === 0 && outsideAssignments.length === 0 ? (
            ...
            <>
              ...
              <SectionTitle
                text={t(
                  "GENERATION_INVESTMENTS_OUTSIDE_ASSIGNMENTS_TABLE_TITLE",
                )}
              />
              <GenerationTable
              ...
              <SectionTitle
                text={t("GENERATION_INVESTMENTS_LINK_TO_PROD_CONSUMPTION")}>
                <Button
                  sx={{
                    backgroundColor: "primary.extraLight",
                    color: "primary.main",
                  ...
                  onClick={() =>
                    window.location.replace(
                      "/investments/production-consumption",
                    )
                  }
