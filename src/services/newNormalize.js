...
export const normalizeSelfconsumption = (selfconsumption) => {
  let data = {
    ...
    installation_power: String(
      Math.round(selfconsumption.installation_power * 1000),
    ),
...
export const newNormalizeContract = (data, gurbCode) => {
  ...
  const finalContract = {
    linked_member:
      data.member.link_member || data.has_member === "campaign-offer"
  ...
  if (data.has_light === "light-off") {
    finalContract["contract_info"]["phase"] =
      data.contract.phase === "mono" ? "230" : "3x230/400"
  }
