...
const sanitizeData = (data) => {
  Object.keys(data).forEach(
    (key) =>
      // NOTE: disable eslint because == is checking NULL and Undefined
...
export const normalizeHolderChange = (
  contract,
...
export const normalizeContract = (contract) => {
  ...
  finalContract.cups_state_id = contract?.supply_point?.state?.id
    ...
    : 0
  finalContract.cups_cadastral_reference = contract?.supply_point
    ?.cadastral_reference
  ...
  finalContract.particular_contract_terms_accepted =
    contract?.particular_contract_terms_accepted

  if (contract?.self_consumption?.have_installation) {
    finalContract.self_consumption = {}
    finalContract.self_consumption.cau =
      contract?.self_consumption?.cau.replace(/ /g, "")
...
export const newNormalizeMember = (data) => {
  ...
  finalMember.tipuspersona =
    data.new_member.person_type === "physic-person"
      ? USER_TYPE_PERSON
      : USER_TYPE_COMPANY
  ...
  finalMember.adreca =
    `${data.address.street}, ${data.address.number} ${data.address.bloc} ${data.address.floor} ${data.address.door}`.trim()
  ...
  if (data.new_member.person_type === "physic-person") {
    ...
    finalMember.birthdate = data.new_member.birthdate
...
export const testPowerForPeriods = (
  ...
  for (let i = 1; i <= rates[rate]?.num_power_periods; i++) {
    ...
    if (limit.match("min")) {
      ...
      inLimit =
        rate === "2.0TD" ? values[attr] <= rates[rate][limit]?.power : true
    }
...
export const newTestPowerForPeriods = (
  ...
  const message = !limit.match("min")
    ? `POWER_NO_MORE_THAN${value}`
    : newRates[rate]?.num_power_periods >
        newRates[rate][limit]?.num_periods_apply
