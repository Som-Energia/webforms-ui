export const GURB_REQUIREMENTS_STEP = "GURB_REQUIREMENTS_STEP"
export const GURB_FINAL_STEP = "GURB_FINAL_STEP"
export const NEW_MEMBER_STEP = "NEW_MEMBER_STEP"
export const NEW_CONTRACT_STEP = "NEW_CONTRACT_STEP"

// GURB form steps
export const GURB_FORM_STEPS = [GURB_REQUIREMENTS_STEP, GURB_FINAL_STEP]

// GURB form validation steps
export const GURB_FORM_REQUIREMENTS_STEPS = [GURB_REQUIREMENTS_STEP]

// GURB form join steps
export const GURB_FORM_JOIN_STEPS = [GURB_FINAL_STEP]

export const GURB_REQUIREMENTS_FORM_SUBSTEPS = {
  SUPPLY_POINT: 0,
  ADDRESS: 1,
  LIGHT: 2,
  TARIFF_SELECTION: 3,
}

// New Contract with Member form steps
export const CONTRACT_MEMBER_FORM_STEPS = [NEW_MEMBER_STEP, NEW_CONTRACT_STEP]

export const CONTRACT_FORM_SUBSTEPS = {
  IDENTIFY_HOLDER: 3,
  HOLDER_INFO: 4,
  SUPPLY_ADDRESS: 5,
  SUPPLY_INFO: 6,
  POWER: 7,
  TARIFF: 8,
  DONATION: 9,
  IBAN: 10,
  SUMMARY: 11,
}

export const NEW_MEMBER_FORM_SUBSTEPS = {
  IDENTIFY_MEMBER: 0,
  MEMBER_INFO: 1,
  PAYMENT_INFO: 2,
  SUMMARY: 3,
}

export const NEW_MEMBER_CONTRACT_FORM_SUBSTEPS = {
  IDENTIFY_MEMBER: 1,
  MEMBER_INFO: 2,
  SUPPLY_POINT: 3,
  SUPPLY_INFO: 4,
  POWER: 5,
  SELFCONSUMPTION: 6,
  SELFCONSUMPTION_INFO: 7,
  HOLDER_INFO: 8,
  DONATION: 9,
  PAYMENT_INFO: 10,
  SUMMARY: 11,
  SIGNATURE: 12,
}

export const NEW_LINK_MEMBER_CONTRACT_FORM_SUBSTEPS = {
  LINK_MEMBER: 1,
  SUPPLY_POINT: 2,
  SUPPLY_INFO: 3,
  POWER: 4,
  SELFCONSUMPTION: 5,
  SELFCONSUMPTION_INFO: 6,
  HOLDER_INFO: 7,
  MEMBER_INFO: 8,
  DONATION: 9,
  PAYMENT_INFO: 10,
  SUMMARY: 11,
  SIGNATURE: 12,
}

export const NEW_HOLDER_CHANGE_FORM_SUBSTEPS = {
  SUPPLY_POINT: 0,
  IDENTIFY_HOLDER: 1,
  HOLDER_INFO: 2,
  MEMBER_INFO: 3,
  ESPECIAL_CASES: 4,
  DONATION: 5,
  PAYMENT_INFO: 6,
  SUMMARY: 7,
  SIGNATURE: 8,
}
